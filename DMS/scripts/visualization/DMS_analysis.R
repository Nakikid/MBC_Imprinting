...
print(p)
ggsave("./DMS/figures/Fig2A.UMAP_mAb.pdf", plot = p, width = 5.5, height = 5)
...
print(p)
ggsave("./DMS/figures/Fig2B.UMAP_mAb_BF7NT50.pdf", plot = p, width = 5.5, height = 5)
...
print(p)
ggsave("./DMS/figures/Fig2B.UMAP_mAb_AncestralNT50.pdf", plot = p, width =5.5, height = 5)
...
print(p)
ggsave("./DMS/figures/Fig2C.Barplot_BindingType_bycluster.pdf", plot = p, width = 4, height = 3)
...
p <- ggplot(tt,aes(y = Freq, x = factor(cluster), fill = sample)) + geom_bar(stat="identity")+
  ...
  theme(axis.text.x=element_text(angle = 45,vjust = 1,hjust = 1,colour="black", size = rel(1.1)),axis.line.x = element_line(colour = "black", size = 0.5),
        ...
        legend.title = element_text(size = 10),panel.grid = element_blank(),panel.background = element_rect(fill = "white", color = NA))
print(p)
ggsave("./DMS/figures/Fig2C.Barplot_Neut_bycluster.pdf", plot = p, width = 4.3, height = 3)
...
p <- ggplot(tt,aes(y = Freq, x = sample, fill = cluster)) + geom_bar(stat="identity")+
  ...
  theme(axis.text.x=element_text(angle = 45,vjust = 1,hjust = 1,colour="black", size = rel(1.1)),axis.line.x = element_line(colour = "black", size = 0.5),
        ...
        legend.title = element_text(size = 10),panel.grid = element_blank(),panel.background = element_rect(fill = "white", color = NA))
print(p)
ggsave("./DMS/figures/Fig2F.Barplot_cluster_bysample.pdf", plot = p, width = 4, height = 3)
...
p <- ggplot(tt,aes(y = Freq, x = sample, fill = cluster)) + geom_bar(stat="identity")+
  ...
  theme(axis.text.x=element_text(angle = 45,vjust = 1,hjust = 1,colour="black", size = rel(1.1)),axis.line.x = element_line(colour = "black", size = 0.5),
        ...
        legend.title = element_text(size = 10),panel.grid = element_blank(),panel.background = element_rect(fill = "white", color = NA))
print(p)
ggsave("./DMS/figures/Fig2E.Barplot_epitope_distribution_inCrossBF7.pdf", plot = p, width = 4.5, height = 4)
...
for (cluster in c("A", "B", "C", "D1", "D2", "E2", "E3", "F1", "F2", "F3")) {
  ...
  ggsave(paste0("./DMS/figures/FigS2D.Mut_Trend/", cluster, ".pdf"), plot = p[[cluster]], width = 8, height = 3)
}
...
data <- merge(data[data$antibody %in% mAb_choose,], mAb_embed[c(1,4:22)], by = "antibody")
...
pdf("./DMS/figures/Fig2D.Heatmap/Heatmap_mAb_site_score.pdf",width = 30,height = 12)
